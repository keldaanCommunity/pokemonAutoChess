...
export async function openBoosterForUser(
    ...
    for (let card of boosterContent) {
        ...
        if (!existingItem) {
            ...
            if (hasUnlocked) {
                ...
                CollectionUtils.unlockEmotion(existingItem.unlocked, card.emotion, card.shiny)
...
export function checkTitlesAfterEmotionUnlocked(
    ...
    if (!mongoUser.titles.includes(Title.DUKE)) {
        if (
            Object.values(Pkm)
                .filter((p) => NonPkm.includes(p) === false && PkmAltForms.includes(p) === false)
                .every((pkm) => {
