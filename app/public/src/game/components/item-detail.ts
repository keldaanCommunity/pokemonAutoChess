  ...
  constructor(scene: Phaser.Scene, x: number, y: number, name: string) {
    ...
    this.dom.innerHTML = `
      ...
      ${Object.keys(ItemRecipe).reduce((prev, item, idx) => {
        ...
        if (recipe[0] == name) {
          content +=
            '<div style="display:flex; flex-flow: column; justify-content:space-between;"><img style="height:18px; width:18px;" src="assets/item/' +
            recipe[1] +
            '.png"/><p style="margin-bottom:0px">=</p><img style="height:18px; width:18px;" src="assets/item/' +
            item +
            '.png"/></div>'
