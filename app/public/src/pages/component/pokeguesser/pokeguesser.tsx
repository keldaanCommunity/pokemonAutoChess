...
export default function Pokeguesser(props: {
    ...
    const [pokemonToGuess, setPokemonToGuess] = useState<Pokemon>(pickRandomIn(listPokemonsToGuess))
    ...
    return (
        ...
        >
            ...
            <div className="attempts">
                ...
                <ul>
                    {attempts.map((pkm, i) => (<PokemonAttempt key={i} index={i} pokemon={pkm} solution={pokemonToGuess} />))}
                </ul>
            </div>
        </Modal>
    )
...
export function PokemonSelect({ value, setValue, onSubmit }: PokemonSelectProps) {
    const { t } = useTranslation();
    ...
    return (
        <form onSubmit={handleSubmit} id="pokemon-select-form">
            ...
            <button className="bubbly blue" type="submit">{t("pokeguessr.guess")}</button>
        </form>
    );
}

export function PokemonAttempt({ pokemon, solution, index }: { pokemon: Pokemon, solution: Pokemon, index: number }) {
    const { t } = useTranslation();
    ...
    return (
        <li className="pokemon-attempt">
            ...
            <span className={cc("rarity", { valid: pokemon.rarity === solution.rarity })}>{t(`rarity.${pokemon.rarity}`)}</span>
            <span className={cc("stars", { valid: pokemon.stars === solution.stars })}> {Array.from({ length: pokemon.stars }, (_, i) => (
                <img src="assets/ui/star.svg" height="24" key={"star" + i}></img>
            ))}</span>
            {index >= 8 && <span className={cc("pool", { valid: pokemon.regional === solution.regional && pokemon.additional === solution.additional })}>{t(`pool.${pokemon.regional ? 'regional' : pokemon.additional ? 'additional' : 'regular'}`)}</span>}
            {values(pokemon.types).map((type, i) => (
                <span key={i} className={cc("type", type, { valid: solution.types.has(type) })}>
