    ...
    render(){
        ...
        return <div style={{backgroundColor:'rgba(255,255,255,1)', margin:'10px'}} className='nes-container'>
            <div style={{display:'flex', justifyContent:'space-between'}}>
                <h3>{this.capitalizeFirstLetter(sortedTypes[0])} {this.props.team.types[sortedTypes[0]]} / {this.capitalizeFirstLetter(sortedTypes[1])} {this.props.team.types[sortedTypes[1]]}</h3>
                <div style={{display:'flex'}}>
                    {sortedTypes.map(type=>{
                        return <div style={{display:'flex', flexFlow:'column', alignItems: 'center'}} key={type}>
                            <img style={{width:'51px', height:'51px', imageRendering:'pixelated'}} src={'assets/types/' + type.toUpperCase() + '.png'}/>
