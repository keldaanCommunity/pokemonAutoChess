...
export default function TestAnim({
    ...
    class TestAnimScene extends Phaser.Scene {
        ...
        create() {
            ...
            const durationArray = durations[`${PKM_INDEX}/${PokemonTint.NORMAL}/${PokemonActionState.IDLE}/${SpriteType.ANIM}`]
            ...
            const pkmSprite = this.add.sprite(width / 2, height / 2, PKM_INDEX, `${PokemonTint.NORMAL}/${PokemonActionState.IDLE}/${SpriteType.ANIM}/${Orientation.DOWN}/0000`)
            pkmSprite.play(PKM_INDEX);
            pkmSprite.setScale(2).setDepth(3)

            const statusSprite = this.add.sprite(width / 2 + offset[0], height / 2 + offset[1], atlasKey).play(animationKey);
