...
export class Pokemon extends Schema implements IPokemon {
  ...
  killCount: number = 0
  evolutions: Pkm[] = []
  evolutionRule: EvolutionRule = { type: "count", numberRequired: 3 }
  additional = false
  regional = false
  canHoldItems = true
  canBeBenched = true
...
export class Egg extends Pokemon {
  ...
  skill = Ability.DEFAULT
  passive = Passive.EGG
  evolutionRule = { type: "hatch" } satisfies EvolutionRule
  canHoldItems = false
}
...
export class Tympole extends Pokemon {
  ...
  evolutionRule = { type: "hatch" } satisfies HatchEvolutionRule
  hp = 80
  atk = 7
  speed = 49
  def = 6
...
export class Stantler extends Pokemon {
  ...
  evolutionRule = {
    type: "condition",
    condition(pokemon: IPokemon, player: IPlayer, stageLevel: number) {
      return (
        player.map !== (pokemon as Stantler).originalMap && stageLevel >= 20
      )
    }
  } satisfies ConditionEvolutionRule
...
export class Primeape extends Pokemon {
  ...
  evolutionRule = { type: "stack" } satisfies StackEvolutionRule
  stacksRequired = 10
  hp = 240
  atk = 20
  speed = 54
...
export class Tandemaus extends Pokemon {
  ...
  evolutionRule = {
    type: "condition",
    condition: (pokemon, player, stageLevel) => stageLevel >= 14
  } satisfies ConditionEvolutionRule
...
export class MausholdThree extends Pokemon {
  ...
  evolutionRule = {
    type: "condition",
    condition: (pokemon, player, stageLevel) => stageLevel >= 20
  } satisfies ConditionEvolutionRule
  passive = Passive.FAMILY
}
...
export class Gimmighoul extends Pokemon {
  ...
  evolutionRule = {
    type: "condition",
    condition: (pokemon, player) => player.money >= 99
  } satisfies ConditionEvolutionRule
...
export class Espurr extends Pokemon {
  ...
  evolutionRule = {
    type: "count",
    numberRequired: 3,
    divergentEvolution: (pokemon, player, stageLevel: number) => {
      const psychicCount = player.synergies.get(Synergy.PSYCHIC) ?? 0
