...
export async function isPlayerTimeout(presence: Presence, playerId: string): Promise<boolean> {
...
export async function givePlayerTimeout(presence: Presence, playerId: string): Promise<void> {
...
export async function getPendingGame(presence: Presence, playerId: string): Promise<PendingGame | null> {
...
export async function setPendingGame(presence: Presence, playerId: string, gameId: string): Promise<void> {
    ...
    return presence.hset(playerId, PENDING_GAME, [gameId, new Date(Date.now() + 1000 * ALLOWED_GAME_RECONNECTION_TIME).toISOString()].join(","));
}

export async function clearPendingGame(presence: Presence, playerId: string): Promise<boolean> {
