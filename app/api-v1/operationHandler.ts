...
export default class OperationHandler{
    ...
    static getGamesByName(req: Request, res: Response){
        ...
        History.find({name: name}, (err, games) => {
            ...
            if(games && games.length > 0){
                ...
                return res.status(404).json({err: `no games found with name ${name}.`})
            }
        });
    }
    ...
    static getGamesByTime(req: Request, res: Response){
        ...
        History.find({startTime: {$gt: startTime}, endTime: {$lt: endTime}}, undefined, {limit: 10, sort: {startTime: -1}}, (err, games) => {
            if(err){
                return res.status(400).json(err);
            }
            if(games && games.length > 0){
                ...
                return res.status(404).json({err: `no games found between epoch${startTime} and ${endTime}.`})
            }
        });
    }
